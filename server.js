...
// Updated Hotpads-formatted endpoint using live YGL feed
app.get("/hotpads-listings", async (req, res) => {
  try {
    ...
    const transformedListings = listings.map(listing => {
      // Convert YGL fields to Hotpads format
      const propertyType = listing.BuildingType || "Apartment";
      const description = [
        listing.UnitDescription || `${listing.Beds || 0} bed, ${listing.Baths || 0} bath`,
        listing.BuildingDescription || `Well-maintained ${propertyType} in ${listing.Neighborhood || listing.City || 'Boston'}`
      ].filter(Boolean).join('. ');

      return `
      <Listing id="${listing.ID}" type="RENTAL" companyId="EncoreRealty" propertyType="${propertyType}">
        <restrictions>
          <seniorHousing/>
          <studentHousing/>
          <militaryHousing/>
        ...
        <name>Available ${listing.AvailableDate || 'Soon'}: ${listing.Beds || 0}BR ${propertyType} in ${listing.Neighborhood || listing.City}</name>
        ...
        <zip>${listing.Zip}</zip>
        <latitude>${parseFloat(listing.Latitude) || 0}</latitude>
        <longitude>${parseFloat(listing.Longitude) || 0}</longitude>
        <lastUpdated>${new Date(listing.UpdateDate || listing.CreateDate).toISOString()}</lastUpdated>
        <contactName>Encore Realty</contactName>
        <contactEmail>info@allaccessboston.com</contactEmail>
        <contactPhone>617-981-6900</contactPhone>
        <price>${parseFloat(listing.Price) || 0}</price>
        <numBedrooms>${parseFloat(listing.Beds) || 0}</numBedrooms>
        <numFullBaths>${Math.floor(parseFloat(listing.Baths)) || 0}</numFullBaths>
        <numHalfBaths>${(parseFloat(listing.Baths) % 1 > 0 ? 1 : 0)}</numHalfBaths>
        ...
        <status>${listing.Status === "ONMARKET" ? "For Rent" : listing.Status}</status>
        <parking>
          <parkingType>${listing.Parking?.ParkingType || "None"}</parkingType>
        </parking>
        ${listing.Laundry ? `
        <ListingTag type="LAUNDRY">
          <tag>${listing.Laundry}</tag>
        </ListingTag>` : ''}
        ${listing.HeatSource ? `
        <ListingTag type="HEATING_FUEL">
          <tag>${listing.HeatSource}</tag>
        </ListingTag>` : ''}
        ${listing.Pet ? `
        <ListingTag type="PET_POLICY">
          <tag>${listing.Pet}</tag>
        </ListingTag>` : ''}
        ${listing.RentIncludes?.RentInclude ? (
          Array.isArray(listing.RentIncludes.RentInclude) 
            ? listing.RentIncludes.RentInclude.map(include => `
        <ListingTag type="RENT_INCLUDES">
          <tag>${include}</tag>
        </ListingTag>`).join('')
            : `
        <ListingTag type="RENT_INCLUDES">
          <tag>${listing.RentIncludes.RentInclude}</tag>
        </ListingTag>`
        ) : ''}
        ${listing.Photos?.Photo ? (
          Array.isArray(listing.Photos.Photo)
            ? listing.Photos.Photo.map(photo => `
        <ListingPhoto source="${photo}">
          <label>Property Photo</label>
        </ListingPhoto>`).join('')
            : `
        <ListingPhoto source="${listing.Photos.Photo}">
          <label>Property Photo</label>
        </ListingPhoto>`
        ) : ''}
      </Listing>`;
